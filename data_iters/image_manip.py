...
def sample_with_rotation_2(x, center, angle, 
                         box_dim, 
                         fill_value=255, 
                         wraparound=True,
                         stdev_threshold=None,
                         test_stdev=False):
    """ Get a patch from x, with rotation
    
    """
    xc, yc = center
    rows, cols=box_dim
    img_rows, img_cols = x.shape
    max_width = max_height = int(np.sqrt(rows**2+cols**2))
    
    # first get the maximal bounding box
    bbox_sample = extract_with_box(x, center, (max_height, max_width), fill_value)
    
    # center image on sampling point
    # epsilon to avoid naughty numpy rounding behavior
    epsilon = 10e-4
    translation_matrix = np.zeros((3,3),dtype=np.float32)
    np.fill_diagonal(translation_matrix, 1)
    translation_matrix[0,2] = -xc + epsilon
    translation_matrix[1,2] = -yc + epsilon
